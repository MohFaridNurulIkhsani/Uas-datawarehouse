...
$getData=array();
$querylihat="SELECT id_nilaikriteria FROM nilai_customer WHERE id_customer='$a' AND id_jenisbarang='$b'";
...
<form id="form" action="./proses/prosesubah.php" method="POST">
    ...
    <div class="panel-middle">
        <div class="group-input">
            <?php
            $query="SELECT namacustomer FROM customer WHERE id_customer='$a'";
            $execute=$konek->query($query);
            $data=$execute->fetch_array(MYSQLI_ASSOC);
            ?>
            <div class="group-input">
                <label for="jenisbarang">Nama customer</label>
                <input class="form-custom" value="<?php echo $data['namacustomer'];?>" disabled type="text" autocomplete="off" required name="jenisbarang" id="jenisbarang">
            </div>
        </div>
        <div class="group-input">
            <?php
        ...
        <?php
        $query="SELECT namaKriteria,id_nilaicustomer,id_kriteria FROM nilai_customer INNER JOIN kriteria USING(id_kriteria) WHERE id_customer='$a'";
        $execute=$konek->query($query);
        if ($execute->num_rows > 0){
            while($data=$execute->fetch_array(MYSQLI_ASSOC)){
                echo "<div class=\"group-input\">";
                echo "<label for=\"nilai\">$data[namaKriteria]</label>";
                echo "<input type='hidden' value=\"$data[id_nilaicustomer]\" name=\"id[]\">";
