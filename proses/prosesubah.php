...
switch ($op){
    case 'barang':
        ...
        break;
    case 'customer':
        $query="UPDATE customer SET namacustomer='$customer' WHERE id_customer='$id'";
        $crud->update($query,$konek,'./?page=customer');
        break;
    case 'kriteria':
    ...
    case 'nilai':
        $query=null;
        for ($i=0;$i<count($id);$i++){
            $query.="UPDATE nilai_customer SET id_nilaikriteria='$nilai[$i]' WHERE id_nilaicustomer='$id[$i]';";
        }
